...
class Strip
{
  public:
    ...
    void rainbowEffect(uint32_t _seed)
    {
      ...
      if (_seed > lastRender)
      {
        if (effectIndex > 255) effectIndex = 0;
        for (uint16_t i = 0; i < ledChipCount; i++)
        {
          wheelPos = (i * 256 / ledChipCount + effectIndex) & 255;
          if (wheelPos < 85)
          {
            adafruitStrip.setPixelColor(i, wheelPos * 3, 255 - wheelPos * 3, 0);
          }
          else if (wheelPos < 170)
          {
            wheelPos -= 85;
            adafruitStrip.setPixelColor(i, 255 - wheelPos * 3, 0, wheelPos * 3);
          }
          else
          {
            wheelPos -= 170;
            adafruitStrip.setPixelColor(i, 0, wheelPos * 3, 255 - wheelPos * 3);
          }
        }
        effectIndex++;
    ...
    {
      if (effectCode == BOUNCINGBALL) {
        effectDelay = constrain( effectDelay, 5, 50 );
      }
      if (effectCode == FIRE) {
        effectDelay = constrain( effectDelay, 5, 200 );
      }
    }
